...
// --- Results Overview ---
export const ResultsScreen: React.FC<{ scores: ScoreRecord[], onRestart: () => void }> = ({ scores, onRestart }) => {
  return (
    <div className="min-h-screen bg-[#0b1120] text-slate-200 p-8 pb-20">
      <div className="max-w-4xl mx-auto">
        <header className="flex items-center justify-between mb-12 border-b border-slate-700 pb-6">
          <div className="flex items-center gap-4">
             <img src="https://raw.githubusercontent.com/oneintellectual/web/refs/heads/master/logo.png" alt="Logo" className="w-12 h-12 rounded-full" />
             <div>
                <h1 className="text-3xl font-display font-bold text-white">评测结果总览</h1>
                <p className="text-slate-400 text-sm">综合能力详细数据单</p>
             </div>
          </div>
        ...
        {/* Warning Badge */}
        <div className="mb-8 bg-yellow-900/20 border-l-4 border-yellow-500 p-4 rounded text-yellow-200 text-sm">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {scores.map((record, index) => (
            <div 
              key={index}
              className="bg-slate-800/50 border border-slate-700 rounded-lg p-5 flex items-center justify-between hover:border-cyan-500/50 transition-colors"
            >
              <div>
                <div className="text-xs text-cyan-400 font-bold tracking-wider uppercase mb-1">{record.levelName}</div>
                <h3 className="text-lg font-bold text-white">{record.testName}</h3>
              </div>
              <div className="text-right">
                <div className="text-2xl font-display font-bold text-white">
                  {record.displayScore} 
                  <span className="text-sm font-normal text-slate-500 ml-1">{record.unit}</span>
                </div>
              </div>
            </div>
          ))}
        ...
        <div className="mt-12 text-center">
            <p className="text-slate-600 text-xs">Generated by EsportsPro Eval System v1.0</p>
        </div>
      </div>
    </div>
  );
